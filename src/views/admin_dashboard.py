...
import sys
import os
from views.admin_request import AdminRequestManager
from views.admin_documents import AdminDocumentManager
from views.admin_billing import AdminBillingManager
from views.admin_student import AdminStudentManager
from views.admin_user import AdminUserManager

# Add the parent directory to the path to import your modules
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from views.admin_feedback import AdminFeedbackManager
...
class AdminDashboard:
    def __init__(
        ...
        self.user_type = self.user_data.get('user_type', 'admin').lower()
        ...
        self.role_permissions = {
            ...
            'registrar': {
                ...
                'documents': True,
                'requests': True,
                'billing': False,  # Registrar doesn't manage payments
                'feedback': True,
                'users': False  # Registrar doesn't manage users
            },
            'cashier': {
                'dashboard': True,
                'students': False,  # Cashier doesn't manage students
                'documents': False,  # Cashier doesn't manage document types
                'requests': True,  # Cashier needs to see requests for payment
                'billing': True,  # Cashier's main function
                'feedback': False,  # Cashier doesn't manage feedback
                'users': False  # Cashier doesn't manage users
            }
        }
    ...
    def has_permission(self, permission):
        ...
        permissions = self.role_permissions.get(self.user_type, {})
    ...
    def create_ui_elements(self):
        ...
        # Admin info - show role based on user type
        admin_name = self.user_data.get('username', 'Admin')
        role_display = self.user_type.title() if self.user_type else 'Admin'
        self.canvas.create_text(
            128.0,
            193.0,
            anchor="nw",
    ...
    def create_navigation_buttons(self):
        ...
        # Students button - only show if user has permission
        if self.has_permission('students'):
            ...
            self.button_students = None

        # Documents button - only show if user has permission
        if self.has_permission('documents'):
            ...
            self.button_documents = None

        # Requests button - only show if user has permission
        if self.has_permission('requests'):
        ...
        # Billing button - only show if user has permission
        if self.has_permission('billing'):
            ...
            self.button_billing = None

        # Feedback button - only show if user has permission
        if self.has_permission('feedback'):
            ...
            self.button_feedback = None

        # Users button - only show if user has permission
        if self.has_permission('users'):
            ...
            self.button_users = None
        
        # Adjust button positions based on which buttons are visible
        self.adjust_button_positions()
    
    def adjust_button_positions(self):
        """Adjust button positions to remove gaps for hidden buttons"""
        y_start = 286.0  # Dashboard button position (always visible)
        y_spacing = 60.0  # Space between buttons
        current_y = y_start + y_spacing  # Start after dashboard button
        
        # List of buttons in order (excluding dashboard which is always first)
        buttons = [
            ('students', self.button_students),
            ('documents', self.button_documents),
            ('requests', self.button_requests),
            ('billing', self.button_billing),
            ('feedback', self.button_feedback),
            ('users', self.button_users)
        ]
        
        # Move visible buttons to fill gaps (only reposition buttons that are visible)
        for permission, button in buttons:
            if button is not None:
                button.place(x=49.0, y=current_y, width=239.0, height=47.0)
                current_y += y_spacing
    ...
    def show_students(self):
        ...
        if not self.has_permission('students'):
            messagebox.showwarning("Access Denied", "You don't have permission to access Students management.")
            return
    ...
    def show_documents(self):
        ...
        if not self.has_permission('documents'):
            messagebox.showwarning("Access Denied", "You don't have permission to access Document Types management.")
            return
    ...
    def show_requests(self):
        ...
        if not self.has_permission('requests'):
            messagebox.showwarning("Access Denied", "You don't have permission to access Requests management.")
            return
    ...
    def show_billing(self):
        ...
        if not self.has_permission('billing'):
            messagebox.showwarning("Access Denied", "You don't have permission to access Billing management.")
            return
    ...
    def show_feedback(self):
        ...
        if not self.has_permission('feedback'):
            messagebox.showwarning("Access Denied", "You don't have permission to access Feedback management.")
            return
    ...
    def show_users(self):
        ...
        if not self.has_permission('users'):
            messagebox.showwarning("Access Denied", "You don't have permission to access User management.")
            return
