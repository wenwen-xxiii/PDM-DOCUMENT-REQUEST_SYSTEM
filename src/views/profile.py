...
class ProfileWindow:
    ...
    def limit_contact_input(self, event=None):
        """Limit contact number input to 11 digits only"""
        if event.char and event.char.isdigit():
            # Get current value
            current = self.entry_contact.get().strip()
            digits_only = ''.join(char for char in current if char.isdigit())
            
            # If already 11 digits, prevent more input
            if len(digits_only) >= 11:
                return "break"
        elif event.char and not event.char.isdigit() and event.keysym not in ['BackSpace', 'Delete', 'Left', 'Right', 'Tab', 'Home', 'End']:
    ...
    def validate_contact_number(self, event=None):
        ...
        # Check if valid (11 digits starting with 09)
    ...
    def setup_input_fields(self):
        ...
        # Add validation for contact number (11 digits only)
        self.entry_contact.bind('<KeyRelease>', self.validate_contact_number)
        # Limit input to 11 digits maximum
        self.entry_contact.bind('<KeyPress>', self.limit_contact_input)
        ...
        # Years from latest to oldest (must be 18+), so from (current_year - 18) down to (current_year - 100)
        # This ensures users can only select years that make them at least 18 years old
        min_age = 18
        max_age = 100
        # Start from (current_year - 18) and go backwards to (current_year - 100)
        year_values = [str(y) for y in range(current_year - min_age, current_year - max_age - 1, -1)]
        self.dob_year = ttk.Combobox(self.main_frame, values=year_values,
    ...
    def save_data(self):
        ...
        try:
            ...
            # Parse date of birth with age validation (must be 18+)
            birth_date = None
            if self.dob_month.get() and self.dob_day.get() and self.dob_year.get():
                try:
                    ...
                    birth_year = int(self.dob_year.get())
                    birth_day = int(self.dob_day.get())
                    
                    # Validate age (must be at least 18 years old)
                    birth_date_obj = datetime.datetime(birth_year, month_num, birth_day)
                    today = datetime.datetime.now()
                    age = today.year - birth_year - ((today.month, today.day) < (birth_date_obj.month, birth_date_obj.day))
                    
                    if age < 18:
                        messagebox.showerror("Age Validation", "You must be at least 18 years old to use this system.")
                        return False
                    
                    birth_date = f"{birth_year}-{month_num:02d}-{birth_day:02d}"
                except ValueError as e:
                    messagebox.showerror("Invalid Date", f"Invalid date of birth: {str(e)}")
                    return False
