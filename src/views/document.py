...
from views.requestform import DocumentRequestWindow
...
class DocumentWindow:
    def __init__(self, parent, user_data=None, user_type=None, get_db_connection=None, navigation_callbacks=None):
        ...
        # Get student_id immediately and cache it
        self.student_id = self._get_student_id_fast()
        self.current_page = 1
        ...
        self._setup_ui()
        self.load_document_requests_fast()
        self.update_display()

    def _get_student_id_fast(self):
        """Get student_id from user_data - OPTIMIZED FOR SPEED"""
        # Method 1: Try to get student_id directly from user_data
        student_id = self.user_data.get('student_id')
        if student_id:
            return student_id
        
        # Method 2: Try to get from user_id lookup (synchronous for speed)
        ...
        # Method 3: Try to get from student_number lookup (synchronous for speed)
        ...
        if student_number:
            try:
                ...
                print(f"Error getting student_id from student_number: {e}")
    ...
    def load_document_requests_fast(self):
        """Load document requests from database - OPTIMIZED FOR SPEED"""
        try:
            ...
            # Always get a fresh database connection to avoid caching issues
            db_connection = self.get_db_connection()
            ...
            # Single optimized query to get all document requests
            # Force fresh query by using NOW() to prevent any caching
            cursor.execute("""
                SELECT 
                    dr.request_id,
                    dr.request_number,
            ...
            # Close cursor and connection immediately to ensure fresh data next time
            cursor.close()
            db_connection.close()
            
            print(f"✅ Loaded {len(self.document_requests)} document requests (fresh from DB)")
            
            # Debug: Print status of first request if any
            if self.document_requests:
                first_req = self.document_requests[0]
                print(f"   First request: {first_req.get('request_number')} - Status: {first_req.get('status')} - Payment: {first_req.get('payment_status')}")
            
        except Exception as e:
            print(f"❌ Error loading document requests: {str(e)}")
            import traceback
            traceback.print_exc()
            # Don't show error messagebox here to avoid interrupting user
            self.document_requests = []
    ...
    def _populate_row_data(self, row_index, request):
        ...
        request_num = request.get('request_number', 'N/A')
        print(f"   Populating row {row_index}: {request_num} - Status: {status}, Payment: {payment_status}")
        ...
        for field_name, value in data_mapping:
            try:
                ...
                row[field_name].insert(0, str(value))
    ...
    def view_document(self, row_index):
        """View document details and attachments - OPTIMIZED FOR SPEED"""
        start_idx = (self.current_page - 1) * self.requests_per_page
        ...
        if actual_index < len(self.document_requests):
            ...
            # Check if document has attachments using synchronous connection
            try:
                db_connection = self.get_db_connection()
                if db_connection:
                    cursor = db_connection.cursor()
    ...
    def refresh_requests(self):
        """Refresh the document requests list - OPTIMIZED FOR SPEED"""
        print("🔄 Refreshing document requests...")
        # Clear existing data first to force fresh load
        self.document_requests = []
        # Reload from database
        self.load_document_requests_fast()
        # Reset to first page to show latest requests
        self.current_page = 1
        # Force update display
        self.update_display()
        # Force UI update
        self.main_frame.update_idletasks()
        print(f"✅ Document requests refreshed - {len(self.document_requests)} requests loaded")
    
    def refresh_if_needed(self):
        """Refresh data when window is being shown after being hidden"""
        # This method is called when switching back to the document window
        # to ensure data is up-to-date with latest database changes
        try:
            print("🔄 Refreshing document requests on navigation...")
            # Force reload data from database to get latest status
            ...
            # Force UI update
            self.main_frame.update_idletasks()
            print(f"✅ Document requests refreshed successfully - {len(self.document_requests)} requests loaded")
        except Exception as e:
            print(f"❌ Error refreshing document data: {e}")
            import traceback
            traceback.print_exc()
            # Try to show current data even if refresh fails
            try:
                self.update_display()
            except:
                pass
