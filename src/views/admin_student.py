...
class AdminStudentManager:
    ...
    def open_student_form(self, student=None):
        ...
        canvas.create_text(
            80, field_y_start + (field_spacing * 5),
            text="Address:", fill="#000000", font=("Inter", 11, "bold"), anchor="nw"
        )
        from tkinter import Text
        ...
        # Obligations section - aligned with Address field
        from tkinter import Checkbutton, BooleanVar
        has_obligations_var = BooleanVar()
        has_obligations_check = Checkbutton(
            dialog,
            text="Has Obligations",
            variable=has_obligations_var,
            font=("Inter", 11, "bold"),
            bg="#FFFFFF",
            command=lambda: obligations_text.config(state="normal" if has_obligations_var.get() else "disabled")
        )
        has_obligations_check.place(x=80, y=field_y_start + (field_spacing * 6) + 20, width=180, height=25)
        
        canvas.create_text(
            80, field_y_start + (field_spacing * 7),
            text="Obligations Details:", fill="#000000", font=("Inter", 11, "bold"), anchor="nw"
        )
        obligations_text = Text(dialog, font=("Inter", 10), width=35, height=2, bg="#FFFFFF", relief="solid", bd=1)
        obligations_text.place(x=80, y=field_y_start + (field_spacing * 7) + 20, width=340, height=40)
        obligations_text.config(state="disabled")
        ...
        if student:
            ...
            has_obligations_var.set(student['has_obligations'] or False)
            if student['obligations_details']:
                obligations_text.insert("1.0", student['obligations_details'])
                obligations_text.config(state="normal" if student['has_obligations'] else "disabled")
        ...
        def save_student():
            ...
            def save_thread():
                try:
                    if student:
                        result = safe_async_run(self.update_student_async, student['student_id'], first_name, last_name, middle_name, course, year_level, contact_number, address, has_obligations, obligations_details)
                    else:
                        result = safe_async_run(self.add_new_student_async, student_number, first_name, last_name, middle_name, course, year_level, contact_number, address, email, has_obligations, obligations_details)
                except Exception as e:
        ...
        Button(dialog, text="Save", font=("Inter", 12, "bold"), bg="#28a745", fg="#FFFFFF", 
               relief="flat", command=save_student).place(x=140, y=730, width=100, height=35)
        Button(dialog, text="Cancel", font=("Inter", 12, "bold"), bg="#6c757d", fg="#FFFFFF", 
               relief="flat", command=cancel_form).place(x=260, y=730, width=100, height=35)
    ...
    async def update_student_async(self, student_id, first_name, last_name, middle_name, course, year_level, contact_number, address, has_obligations=False, obligations_details=""):
        ...
        try:
            ...
            cursor.execute("""
                ...
                SET first_name = %s, last_name = %s, middle_name = %s, course = %s, 
                    ...
                    has_obligations = %s, obligations_details = %s
                WHERE student_id = %s
            """, (first_name, last_name, middle_name, course, year_level, contact_number, address, has_obligations, obligations_details, student_id))
    ...
    def update_student(self, student_id, first_name, last_name, middle_name, course, year_level, contact_number, address, has_obligations=False, obligations_details=""):
        ...
        try:
            ...
            try:
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(safe_async_run, self.update_student_async, student_id, first_name, last_name, middle_name, course, year_level, contact_number, address, has_obligations, obligations_details)
                    return future.result()
            except Exception as e:
                print(f"❌ Error in async update_student: {e}")
                return safe_async_run(self.update_student_async, student_id, first_name, last_name, middle_name, course, year_level, contact_number, address, has_obligations, obligations_details)
        else:
            # Event loop exists, run in thread pool
            try:
                with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    async def add_new_student_async(self, student_number, first_name, last_name, middle_name, course, year_level, contact_number, address, email, has_obligations=False, obligations_details=""):
    ...
    def add_new_student(self, student_number, first_name, last_name, middle_name, course, year_level, contact_number, address, email, has_obligations=False, obligations_details=""):
        ...
        try:
            ...
            try:
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(safe_async_run, self.add_new_student_async, student_number, first_name, last_name, middle_name, course, year_level, contact_number, address, email, has_obligations, obligations_details)
                    return future.result()
            except Exception as e:
                print(f"❌ Error in async add_new_student: {e}")
                return safe_async_run(self.add_new_student_async, student_number, first_name, last_name, middle_name, course, year_level, contact_number, address, email, has_obligations, obligations_details)
        else:
            # Event loop exists, run in thread pool
            try:
                with concurrent.futures.ThreadPoolExecutor() as executor:
