...
class SignupWindow:
    ...
    async def attempt_signup_async(self):
        ...
        # Check for Gmail typos FIRST before exact Gmail validation
        email_lower = email.lower()
        
        # Common Gmail typos to catch: @gmai.com, @gmail.co, @gmai.co, @gmail.c, etc.
        gmail_typos = [
            '@gmai.com', '@gmail.co', '@gmai.co', '@gmail.c', '@gmai.c',
            '@gmaill.com', '@gmail.comm', '@gmial.com', '@gmal.com'
        ]
        
        # Check if email looks like a Gmail typo
        for typo in gmail_typos:
            if email_lower.endswith(typo):
                error_msg = (
                    f"Invalid email address format.\n\n"
                    f"You entered: {email}\n\n"
                    f"Did you mean: @gmail.com?\n\n"
                    f"Please check your email address and correct the typo."
                )
                await self.run_in_main_thread(
                    lambda msg=error_msg: messagebox.showerror("Email Error", msg)
                )
                await self.run_in_main_thread(lambda: self.entry_email.focus())
                return
        
        # Check Gmail format FIRST if it's a Gmail address
        if email_lower.endswith('@gmail.com'):
            # Validate Gmail format first before general email validation
            ...
            # Check for invalid characters in Gmail local part
            # Gmail allows: letters, numbers, dots, hyphens, and underscores
            if not re.match(r'^[a-z0-9._-]+$', local_part):
                await self.run_in_main_thread(
                    lambda: messagebox.showerror(
                        "Email Error",
                        "Invalid Gmail address format.\n\n"
                        "Gmail addresses can only contain:\n"
                        "• Letters (a-z)\n"
                        "• Numbers (0-9)\n"
            ...
            # Check for consecutive dots (not allowed in Gmail)
            if '..' in local_part:
                await self.run_in_main_thread(
                    lambda: messagebox.showerror(
                        "Email Error",
            ...
            # Check if email starts or ends with dot (not allowed in Gmail)
        ...
        # General email validation (after Gmail-specific checks)
        ...
        # Validate password strength: must contain 0-9, A-Z, a-z, and special characters
        password_errors = []
        ...
        if password_errors:
            ...
            await self.run_in_main_thread(
                lambda msg=error_message: messagebox.showerror("Password Error", msg)
            )
            await self.run_in_main_thread(lambda: self.entry_pass.focus())
            return
        ...
        try:
            ...
            if email_sent:
                ...
                # Enhanced error message for Gmail addresses
                if email.lower().endswith('@gmail.com'):
                    error_message = (
                        "Failed to send OTP to your Gmail address.\n\n"
                        "Possible reasons:\n"
                        "• Email may be in spam/junk folder\n"
                        "• Gmail security settings blocking the email\n"
                ...
                await self.run_in_main_thread(
                    lambda: messagebox.showerror("Email Error", error_message)
                )
    ...
    def _send_otp_email(self, email, otp_code):
        ...
        try:
            ...
            result = email_service.send_otp_email_sync(email, otp_code)
            
            # Additional error handling for Gmail addresses
            if not result and email.lower().endswith('@gmail.com'):
                # Log Gmail-specific error for debugging
                print(f"[WARNING] Failed to send OTP to Gmail address: {email}")
                print("[INFO] This could be due to:")
                print("  - Gmail spam filters blocking the email")
            ...
            # Enhanced error handling for Gmail
            if email.lower().endswith('@gmail.com'):
                print(f"[ERROR] Exception sending OTP to Gmail address {email}: {str(e)}")
            return False
