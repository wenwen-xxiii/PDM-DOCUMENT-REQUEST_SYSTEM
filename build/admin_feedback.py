...
from tkinter import Canvas, Frame, Label, Button, Entry, StringVar, messagebox, Scrollbar, Toplevel, Text
...
class AdminFeedbackManager:
    ...
    def load_feedbacks(self):
        ...
        try:
            ...
            messagebox.showerror("Database Error", f"Failed to load feedbacks: {str(e)}")
            self.feedbacks = []
    ...
    def create_table_row(self, row_index, feedback):
        ...
        student_display = "Anonymous" if feedback['is_anonymous'] else feedback['student_name']
        ...
        text_configs = [
            ...
            (240.0, student_display[:15] + "..." if len(student_display) > 15 else student_display, "center"),
    ...
    def view_feedback_details(self, feedback):
        ...
        # Title
        title_label = Label(details_frame, text="Feedback Details", 
        ...
        info_labels = [
            ...
            ("Rating:", rating_stars),
            ("Date:", feedback['created_at'].strftime('%Y-%m-%d %H:%M:%S') if feedback['created_at'] else "N/A"),
        ]
        ...
        for label_text, value_text in info_labels:
            ...
            value = Label(frame, text=value_text, font=("Inter", 10), 
                         bg="#FFFFFF", fg="#666666", anchor="w", wraplength=400)
        ...
        comments_label = Label(comments_frame, text="Comments:", font=("Inter", 10, "bold"), 
                              bg="#FFFFFF", fg="#333333")
        comments_label.pack(anchor="w")
        
        comments_text = Text(comments_frame, font=("Inter", 10), width=60, height=4, 
        ...
        suggestions_label = Label(suggestions_frame, text="Suggestions:", font=("Inter", 10, "bold"), 
                                bg="#FFFFFF", fg="#333333")
        suggestions_label.pack(anchor="w")
        
        suggestions_text = Text(suggestions_frame, font=("Inter", 10), width=60, height=4, 
                               wrap="word", state="disabled", bg="#F8F8F8")
        suggestions_text.pack(fill="x", pady=(5, 0))
        suggestions_text.config(state="normal")
        suggestions_text.insert("1.0", feedback['suggestions'] or "No suggestions provided.")
        ...
        if feedback['responded_to'] and feedback['response']:
            ...
            response_label = Label(response_frame, text="Admin Response:", font=("Inter", 10, "bold"), 
                                  bg="#FFFFFF", fg="#333333")
            response_label.pack(anchor="w")
            
            response_text = Text(response_frame, font=("Inter", 10), width=60, height=4, 
                                wrap="word", state="disabled", bg="#E8F5E8")
            ...
            # Response info
            responder_name = f"{feedback['responded_by_first']} {feedback['responded_by_last']}" if feedback['responded_by_first'] else "Admin"
            response_date = feedback['responded_at'].strftime('%Y-%m-%d %H:%M:%S') if feedback['responded_at'] else "N/A"
            
            response_info = Label(response_frame, text=f"Responded by: {responder_name} on {response_date}", 
                                font=("Inter", 9), bg="#FFFFFF", fg="#666666")
    ...
    def respond_to_feedback(self, feedback):
        ...
        # Title
        title_label = Label(dialog, text="Respond to Feedback", 
                           font=("Inter", 14, "bold"), bg="#FFFFFF", fg="#792D1B")
        ...
        Label(details_frame, text=f"Request: {feedback['request_number']}", 
              font=("Inter", 10, "bold"), bg="#F8F8F8").pack(anchor="w", padx=10, pady=(10, 5))
        Label(details_frame, text=f"Student: {student_display}", 
              font=("Inter", 10), bg="#F8F8F8").pack(anchor="w", padx=10, pady=2)
        Label(details_frame, text=f"Document: {feedback['document_code']}", 
              font=("Inter", 10), bg="#F8F8F8").pack(anchor="w", padx=10, pady=2)
        Label(details_frame, text=f"Rating: {rating_stars}", 
              font=("Inter", 10), bg="#F8F8F8").pack(anchor="w", padx=10, pady=2)
        
        # Comments display
        Label(details_frame, text="Comments:", font=("Inter", 10, "bold"), 
              bg="#F8F8F8").pack(anchor="w", padx=10, pady=(10, 5))
        comments_text = Text(details_frame, font=("Inter", 9), width=50, height=3, 
                            wrap="word", state="disabled", bg="#FFFFFF")
        ...
        # Response input
        Label(dialog, text="Your Response:", font=("Inter", 10, "bold")).pack(anchor="w", padx=20, pady=(0, 5))
        response_text = Text(dialog, font=("Inter", 10), width=50, height=8, wrap="word")
        ...
        def submit_response():
            ...
            success = self.submit_feedback_response(feedback['feedback_id'], response_content)
        ...
        Button(button_frame, text="Submit Response", font=("Inter", 10, "bold"), bg="#28a745", fg="#FFFFFF", 
               relief="flat", command=submit_response).pack(side="left", padx=(0, 10))
        Button(button_frame, text="Cancel", font=("Inter", 10, "bold"), bg="#6c757d", fg="#FFFFFF", 
               relief="flat", command=cancel_form).pack(side="left")
    ...
    def view_response(self, feedback):
        ...
        # Title
        title_label = Label(dialog, text="Admin Response", 
                           font=("Inter", 14, "bold"), bg="#FFFFFF", fg="#792D1B")
        title_label.pack(pady=20)
        
        # Response content
        response_text = Text(dialog, font=("Inter", 10), width=50, height=12, 
                            wrap="word", state="disabled", bg="#E8F5E8")
        response_text.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        response_text.config(state="normal")
        ...
        info_label = Label(dialog, text=f"Responded by: {responder_name} on {response_date}", 
                          font=("Inter", 9), bg="#FFFFFF", fg="#666666")
    ...
    def show_no_feedbacks_message(self):
        ...
        self.canvas.create_text(
            450, 200, anchor="center", text="No feedback found",
            fill="#666666", font=("Inter", 14, "bold"), tags="row"
        )

    def update_navigation(self):
        """Update navigation elements"""
        total_pages = max(1, (len(self.filtered_feedbacks) + self.feedbacks_per_page - 1) // self.feedbacks_per_page)
